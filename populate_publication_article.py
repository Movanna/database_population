...
# input the parameters for the articles
COLLECTION_ID = 2
...
# populate table publication with articles and create needed content
# in other tables, i.e. translation, translation_text
def create_article_publication(articles):
    directory = XML_OUTPUT_FOLDER
    directory_path = create_directory(directory)
    insert_query = """INSERT INTO publication(publication_collection_id, published, published_by, genre, original_publication_date, original_language) VALUES(%s, %s, %s, %s, %s, %s) RETURNING id"""
    for article in articles:
        published = 1
        original_date = article[1]
        original_publication_date, date_uncertain = replace_date(original_date)
        # all of the articles are in Swedish
        ...
        values_to_insert = (COLLECTION_ID, published, published_by, GENRE, original_publication_date, original_language)
        cursor.execute(insert_query, values_to_insert)
        publication_id = cursor.fetchone()[0]
        # the title of the publication is in swe and fin
        # the titles are kept in a different table
        # title contains the date (almost) as it has been recorded originally
        ...
        create_file(directory_path, original_publication_date, original_language, publication_id, translation_id, original_title, title_swe, newspaper)
        article.extend((publication_id, title_swe))
        print(article)
...
def add_title(publication_id, original_date, date_uncertain, original_title):
    ...
    # since it'll be part of a title
...
def create_file(directory_path, original_publication_date, original_language, publication_id, translation_id, original_title, title_swe, newspaper):
    # files and directories contain the publication's date
    # these publications are articles and files/directories
    # contain the name of the newspaper they were published in
    original_publication_date = original_publication_date.replace("-", "_")
    newspaper = create_title_part_for_file(newspaper)
    directory = directory_path + "/" + newspaper
    newspaper_directory_path = create_directory(directory)
    title_part = create_title_part_for_file(original_title)
    final_directory = newspaper_directory_path + "/" + original_publication_date + "_" + title_part
...
# file and directory names contain the article's title
# with certain replacements
def create_title_part_for_file(title_or_newspaper):
    title_part = title_or_newspaper.replace(". ", "_")
    ...
    # otherwise the file paths may become too long
...
def main():
    # info about publications of type "article"
    articles = create_list_from_csv(CSV_IN)
    articles_with_id = create_article_publication(articles)
    write_list_to_csv(articles_with_id, CSV_OUT)
    conn_db.close()
    cursor.close()
